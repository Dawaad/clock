...
def _render_grid(f, state, cols, rows) -> None:
    ...
    _frame_box(f, x0, y0, x1, y1, top_split, bot_split, midy)
...
def _keybinds_quadrant(f, ax0, ay0, ax1, ay1) -> None:
...
def _frame_box(f, x0, y0, x1, y1, top_split, bot_split, midy) -> None:
    c = theme.FAINT
    for x in range(x0, x1 + 1):
        f.put(x, y0, "─", c)
        f.put(x, y1, "─", c)
